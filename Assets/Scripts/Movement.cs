...
public class Movement : TimeStepMonoBehaviour
{
    ...
    private Vector3 GetInputVector()
    {
        ...
        var fwdMove = _buttonsDown.HasFlag(MovementButtons.Forward)
            ...
            ? -MoveSpeed
            : 0;
        return Camera.transform.TransformDirection(new Vector3(sideMove, 0, fwdMove));
    }

    private void TryMouseLook(Vector2 mouseDelta)
    {
        if(Mode == MovementMode.Editor)
        {
            Cursor.lockState = CursorLockMode.None;
            Cursor.visible = true;
        }
        ...
        if(Mode == MovementMode.Normal && Cursor.lockState != CursorLockMode.Locked)
        {
            return;
        }
        else if(Mode == MovementMode.Editor && !Input.GetKey(KeyCode.Mouse1))
        {
            return;
        }
